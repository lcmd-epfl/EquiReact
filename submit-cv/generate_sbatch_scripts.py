...
for fname in ['gdb_config.dat', 'cyclo_config.dat', 'proparg_config.dat']:
...
python train.py \\"""
    ...
    for invariant in [False, True]:
        ...
        # 2) other splits, dft, noH
        for splitter in ['scaffold', 'yasc', 'ydesc', 'sizeasc', 'sizedesc']:
            for key, config in base_configs.items():
                extra_name = f"{base_config['dataset']}-{'inv-' if invariant else ''}{splitter}-noH-dft-{key}"
                wandb_name = f"cv10-{extra_name}-{base_name}"
                extra_config=f"--splitter {splitter} \\\n--noH \\\n--wandb_name {wandb_name} \\"
                if invariant:
                    extra_config += '\n--invariant \\'
                with open(f'{extra_name}.sbatch', 'w') as f:
                    print("\n".join((header, run_config, config, extra_config)), file=f)
